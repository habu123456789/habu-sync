    ...
    >
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <button
            onClick={() => {
              // Neo-Brutalism themes: swap between light/dark variants
              if (theme === 'nb-light') { setTheme('nb-dark'); return; }
              if (theme === 'nb-dark') { setTheme('nb-light'); return; }
              // Minimal themes: swap between light/dark variants
              if (theme === 'minimal-light') { setTheme('minimal-dark'); return; }
              if (theme === 'minimal-dark') { setTheme('minimal-light'); return; }
              setBlackMode((v) => !v);
            }}
            aria-label={
              theme === 'nb-dark' || theme === 'minimal-dark' ? 'Switch to light mode'
              : theme === 'nb-light' || theme === 'minimal-light' ? 'Switch to dark mode'
            ...
            className={`flex items-center gap-1.5 text-sm px-2.5 py-1.5 rounded-full glass transition-colors ${
              (blackMode || theme === 'nb-dark' || theme === 'minimal-dark') ? 'bg-foreground/10 text-foreground' : 'text-muted-foreground hover:text-primary'
            }`}
          >
            {(theme === 'nb-dark' || theme === 'minimal-dark' || (theme !== 'nb-light' && theme !== 'minimal-light' && blackMode))
              ? <Sun className="w-3.5 h-3.5" />
              : <Moon className="w-3.5 h-3.5" />}
            <span className="hidden sm:inline">
              {theme === 'nb-dark' || theme === 'minimal-dark' ? 'Light' : theme === 'nb-light' || theme === 'minimal-light' ? 'Dark' : (blackMode ? 'Light' : 'Black')}
            </span>
          </button>
          <button
